  ...
  return (
    ...
    >
      ...
      <p className="text-base text-gray-600 sm:text-lg max-w-md">
        We couldn't load <span className="font-medium text-black">{title}</span>{" "}
        at the moment. This could be due to a temporary issue. Please check your
...
const NoVenuesFound = ({ title }: { title: string }) => (
  <div className="flex flex-col items-center justify-center gap-6 px-6 py-12 text-center">
    ...
    <p className="text-base text-gray-600 sm:text-lg max-w-md">
      ...
      <span className="font-medium text-black">{title}</span> available at the
      moment. Try adjusting your search filters or check back later.
    </p>
  </div>
);
