...
export const VenueDetails = ({ venueDetails, vendors, selectedDate }: BookingModalPopupProps) => {
    ...
    <Card className="border-0 shadow-none">
      <CardContent className="p-6">
        <div className="flex border-b border-black/15 gap-4 pb-6">
          ...
          <div>
            <h3 className="font-semibold text-lg">{venueDetails?.venuename}</h3>
            ...
            {selectedDate && (
              <div className="flex items-center gap-2 text-muted-foreground mt-1">
                ...
                <span className="text-sm">Selected Date: {selectedDate.toDateString()}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <p className="text-sm text-primaryDark font-semibold my-6">
          You have chosen the selected vendors for your site visit, you are not charged
          yet.
        </p>
