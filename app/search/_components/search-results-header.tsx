...
export function SearchResultsHeader({
  ...
  return (
    <div className="flex flex-col md:flex-row items-center justify-between gap-4">
      {/* Sort Dropdown */}
      <div className="w-full flex items-center justify-between md:block lg:w-[190px]">
        <div>
        ...
        <div className="md:hidden">
          <button onClick={() => setShowFilterMobile((prev) => !prev)}>
            {!isShowFilter ? (
              <AlignJustifyIcon strokeWidth={1} size={32} color="#868686" />
            ) : (
              <XIcon size={32} strokeWidth={1} color="#868686" />
            )}
          </button>
        </div>
      </div>
      ...
      <div className="w-full hidden sm:block sm:w-auto">
        <Tabs defaultValue="listings" onValueChange={onTabChange}>
          <TabsList className="flex  bg-[#F0F0F0]  shadow-none px-1 rounded-full py-2 h-10 space-x-1 w-full sm:w-auto">
            <TabsTrigger
              value="listings"
              className="flex-1 h-8 text-skin-black rounded-full data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow-[0px_2px_16px 0px_rgba(0, 0, 0, 0.08] border-0 px-4 border-gray-200 sm:flex-none w-fit data-[state=active]:shadow-pill"
            >
              Listings
            </TabsTrigger>
            <TabsTrigger
